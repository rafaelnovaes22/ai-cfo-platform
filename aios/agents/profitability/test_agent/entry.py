...
Responsabilidade única: gerar testes para um módulo já implementado.
...
- docs/specs/_backend_{{module}}.md (output do backend agent, se existir)
...
1. Edge cases declarados na spec — especialmente cenários financeiros, cancelamento,
   integrações externas, permissões
2. Regras de negócio críticas — cada regra com pelo menos 1 caso positivo e 1 negativo
...
- Testes que envolvem moeda devem usar valores extremos (zero, negativo onde proibido,
  máximo da spec, valor mínimo, casas decimais limítrofes)

Não gere mocks para regras de negócio quando a stack permitir teste de integração real.
Gere mocks apenas para serviços externos não controláveis (gateways, APIs de terceiros).
...
class TestAgent(BaseAgent):
    ...
    def run(self, task_input: dict) -> dict:
        ...
        self.messages.append({
            "role": "user",
            "content": (
                f"Gere os testes completos para o módulo: **{module_name}**\n\n"
                f"Spec (edge cases obrigatórios):\n{ctx['spec']}\n\n"
                f"Código backend gerado:\n{ctx['backend'] or '(ainda não disponível — gere testes baseados na spec)'}\n\n"
                f"Use a stack declarada em aios/config.yaml: {_STACK_TESTS}.\n"
                "Priorize cobertura de edge cases financeiros e regras de negócio críticas."
            ),
        })
