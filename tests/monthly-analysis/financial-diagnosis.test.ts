...
describe("inflow_concentration_high — concentração de receita", () => {
  it("high quando cliente único >= 50% das entradas", () => {
    // NOVAIS DIGITAL=600k de 1_000k → 60%
    const anomalies = detectFinancialAnomalies({
      dre: dre({ receitaLiquida: 500_000 }),
      normalizedEntries: [
        entry("NOVAIS DIGITAL", "2026-05-01", "in", 300_000),
        entry("NOVAIS DIGITAL", "2026-05-02", "in", 300_000),
  ...
  it("medium quando cliente único 35-50% das entradas", () => {
    // NOVAIS DIGITAL=400k de 1_000k → 40%
    const anomalies = detectFinancialAnomalies({
      dre: dre({ receitaLiquida: 500_000 }),
      normalizedEntries: [
        entry("NOVAIS DIGITAL", "2026-05-01", "in", 400_000),
