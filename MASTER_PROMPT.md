# Aicfo — Master Prompt do Orchestrator Foundry
**Versão:** 1.0 | **Data:** 2026-05-13 | **Foundry:** v0.15.0 | **Tipo:** agentic_saas / ai_enabled:true

> Prompt de orquestração canônico para operar o Aicfo sob o Novais Digital Foundry.
> Gerado a partir de `templates/master-prompt.md` (Foundry v0.15.0) e adaptado para `project_type: agentic_saas`.
...
```markdown
# Você opera o projeto Aicfo sob o Novais Digital Foundry
...
**Framework**: Novais Digital Foundry v0.15.0 — leia `docs/foundry/project.json` antes de qualquer ação.
...
**NUNCA** promova sem `/novais-digital:promote` e aceite humano.
...
| Intenção | Comando |
|---|---|
| Iniciar SKU novo | `/novais-digital:diagnose` → `/novais-digital:spec` → `/novais-digital:plan` |
| Implementar | `/novais-digital:tasks` → `/novais-digital:implement` |
| Promover lifecycle | `/novais-digital:promote` |
| Auditoria mensal | `/novais-digital:audit-monthly` |
| Validar framework | `bash scripts/foundry-doctor.sh --consumer` |

## 8. Arquivos que NÃO editar sem confirmação

- `.claude/CONSTITUTION.md` — canônica do Foundry
- `docs/adr/*.md` assinados — abrir nova ADR
- `src/skus/*/spec.md` aprovados — reaprovação necessária
- Promoção de SHADOW → ASSISTED → AUTONOMOUS — apenas via `/novais-digital:promote`
```

---

*Gerado a partir de `templates/master-prompt.md` do Foundry v0.15.0. Atualizar ao sincronizar.*
