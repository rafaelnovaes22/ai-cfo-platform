...
import { ThumbsUp, ThumbsDown, Inbox, Zap, Shield, TrendingUp } from "lucide-react";
    ...
    return (
      <div className="space-y-10">
        <header className="animate-fade-up">
          ...
          <h1 className="text-3xl leading-[1.05] tracking-tight">Próximos passos para a sua empresa</h1>
          <div className="mt-4"><AnalysisPicker /></div>
        </header>
        <section className="dark:bg-[#15152f] border dark:border-[#15152f]/50 rounded-lg p-12 text-center">
          <Inbox className="h-10 w-10 mx-auto dark:text-[#96ff7e] mb-4" strokeWidth={1.4} />
          <h2 className="text-[26px] tracking-tight mb-2">Nenhum plano ainda</h2>
          <p className="text-[14px] dark:text-[#96ff7e] max-w-md mx-auto mb-6">
            O plano de ação é gerado automaticamente após a importação dos seus dados.
          </p>
          <Link
            to="/importar"
    ...
    <div className="space-y-10">
      <header className="animate-fade-up flex items-end justify-between gap-6 flex-wrap">
        <div>
          ...
          <h1 className="text-2xl leading-[1.05] tracking-tight">Próximos passos para a sua empresa</h1>
        </div>
        <AnalysisPicker />
      </header>
          ...
          return (
            <button
              ...
              className={`flex !text-left flex-col items-start px-2 md:px-5 pb-3 -mb-px border-b-2 transition-colors ${
                active ? "border-[#3D24A0] dark:border-[#96ff7e]" : "border-transparent opacity-50 hover:"
              }`}
            >
              <span className="md:text-[18px]">
                {h.label} {count > 0 && <span className="text-[10.5px]">({count})</span>}
              </span>
              <span className="text-[11.5px] text-[#3D24A0] dark:text-[#96ff7e]">{h.sub}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    ...
    >
        ...
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-10 relative z-1">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${effortChip[item.effortLevel]}`}>
                <Zap className="h-3 w-3" />
                {effortLabel[item.effortLevel]}
              </span>
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${riskChip[item.riskLevel]}`}>
            ...
            <h3 className="text-[22px] tracking-tight leading-snug mb-2">{item.title}</h3>
            <p className="text-[13.5px] opacity-60 leading-relaxed">{item.description}</p>
          </div>
        </div>
      </div>
      <div className="col-span-12 lg:col-span-3 p-6 lg:border-l border-t lg:border-t-0 dark:border-[#2b2b40] bg-gray-200 dark:bg-[#15152f] flex flex-col gap-3">
        <div className="uppercase text-[11px] tracking-widest mb-1 !opacity-30">Feedback</div>
        <button
        ...
        {item.doneWhen && (
          <p className="text-[11px] opacity-50 mt-auto leading-relaxed">{item.doneWhen}</p>
        )}
      </div>
    </article>
  );
