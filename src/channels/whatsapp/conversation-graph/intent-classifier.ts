...
export function classifyWaIntent(
  ...
  if (["1", "2", "3"].includes(normalizedText)) {
    const intent: WaIntent =
      normalizedText === "3" ? "ASK_MONTHLY_ANALYSIS" : "ASK_CASHFLOW";
  ...
  if (
    ["sim", "ok", "pode", "claro", "vamos", "continuar"].includes(
      normalizedText,
    )
  ) {
    return {
  ...
  if (
    ...
    return {
      ...
      confidence:
        conversation?.lastOutcome || conversation?.passiveContext
          ? "high"
          : "medium",
      requiresSlm: Boolean(
        conversation?.lastOutcome || conversation?.passiveContext,
