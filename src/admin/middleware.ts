...
export async function requireAdminKey(req: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  if (!expected) {
    ...
    await reply.status(503).send(
      problemDetail({
        type: "https://api.aicfo.com.br/errors/admin-not-configured",
  ...
  if (!provided) {
    await reply.status(401).send(
      problemDetail({
        type: "https://api.aicfo.com.br/errors/admin-unauthorized",
  ...
  if (a.length !== b.length || !timingSafeEqual(a, b)) {
    ...
    await reply.status(403).send(
      problemDetail({
        type: "https://api.aicfo.com.br/errors/admin-forbidden",
