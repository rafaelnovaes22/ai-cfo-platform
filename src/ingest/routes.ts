    ...
    handler: async (req, reply) => {
      ...
      if (!data) {
        return reply.status(400).send(
          problemDetail({ type: "https://api.example.com/errors/bad-request", title: "Arquivo não enviado", status: 400 }),
        );
      }
      ...
      if (referenceMonthRaw && !/^\d{4}-(0[1-9]|1[0-2])$/.test(referenceMonthRaw)) {
        return reply.status(400).send(
          problemDetail({ type: "https://api.example.com/errors/bad-request", title: "Query ?referenceMonth deve ser YYYY-MM", status: 400 }),
        );
      }
