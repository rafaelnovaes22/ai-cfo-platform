...
// Na plataforma de deploy, CORS_ORIGIN_PATTERN (regex) libera adicionalmente os
// domínios gerados pelo provedor para frontends de staging/prod mesmo quando
// FRONTEND_ORIGIN está configurado apenas com origens canônicas.
...
app.setErrorHandler(async (err: unknown, req, reply) => {
  ...
  if (isErrorLike(err) && typeof err.statusCode === "number" && err.statusCode < 500) {
    return reply.status(err.statusCode).send(problemDetail({
      type: "https://api.example.com/errors/request-error",
