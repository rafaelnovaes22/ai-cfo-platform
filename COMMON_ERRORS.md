...
### `foundry-doctor` falha em C6 com "trace sem campo obrigatório"
...
### `foundry-doctor` falha em C8 com "aios test_agent sem modes"
**Causa**: `templates/aios/agents/test_agent/config.json.template` não declara `modes: [red, verify]`.
**Solução**: Rodar `foundry-sync` para atualizar o template do canônico:
```bash
bash /caminho/para/agent-governance-framework/scripts/foundry-sync.sh --from /caminho/para/agent-governance-framework
```
...
### `/novais-digital:promote` recusa com "eval suite desatualizada"
**Causa**: O `prompt_hash` no manifest não bate com o hash atual dos prompts (mudança sem re-eval).
**Solução**: Rodar `/novais-digital:eval` para atualizar a suite, depois tentar promover novamente.

---

### `foundry-doctor --consumer` falha em C9 (drift)
...
bash /c/Users/Rafael/Projetos/agent-governance-framework/scripts/foundry-sync.sh --from /c/Users/Rafael/Projetos/agent-governance-framework
