#!/usr/bin/env tsx
/**
 * foundry:doctor — valida a consistência da estrutura Foundry no consumidor (Aicfo).
 *
...
const manifestPath = resolve(root, "docs/foundry/manifest.json");

if (!existsSync(manifestPath)) {
  console.error("❌ foundry:doctor — docs/foundry/manifest.json não encontrado.");
  process.exit(1);
}
...
if (failed > 0) {
  console.error(`\nfoundry:doctor — ${failed} check(s) falharam. Foundry drift detectado.`);
  process.exit(1);
}
console.log(`\nfoundry:doctor — ${checks.length} checks OK.`);
