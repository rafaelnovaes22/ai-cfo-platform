...
//     [--file="./samples/relatorio-financeiro.xlsx"] \
...
const BASE = arg("base", process.env.STAGING_URL ?? "https://staging.example.com");
const FILE = arg("file", "./samples/relatorio-financeiro.xlsx");
const PASSWORD = arg("password", "LoadTest@2026");
const POLL_MS = 3000, MAX_POLL = 200; // ~10 min teto por análise (sob carga, demora mais)

// Guard anti-produção: hosts contendo "api-production" são tratados como PROD
// (hosts de staging são permitidos). Ajuste PROD_URL_PATTERN ao seu ambiente.
if (new RegExp(process.env.PROD_URL_PATTERN ?? "api-production").test(BASE)) {
...
async function oneUser(i, buf) {
  ...
  try {
    const token = await login(`loadtest+${idx}@novais-digital.test`);
