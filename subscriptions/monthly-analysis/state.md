...
foundry_command_version: "promote@0.3.0"
---
...
## 2. Pré-conditions checklist (para `/novais-digital:promote start_shadow`)
...
| 5 | `--approver_po` e `--approver_promotion_officer` declarados (roles distintos) | ⚠️ | founder-solo: Rafael nas duas roles — bypass `NOVAIS_FOUNDRY_BYPASS=incident:founder-solo-pre-team` registrado em §4 |
| 6 | Tracing configurado | ✅ | LangSmith via LangChain configurado em `src/observability/langfuse.ts` |

**Status global**: pre-conditions 1-4 + 6 ✅ satisfeitas (5 via bypass documentado). Pronto para executar `/novais-digital:promote start_shadow` com flag `NOVAIS_FOUNDRY_BYPASS=incident:founder-solo-pre-team`.
...
Por design constitucional, o gate 5 do `/novais-digital:promote` requer `approver_po != approver_promotion_officer` (sem self-approval). Em projeto founder-solo, não há 2ª pessoa para assinar.

### 4.1. Decisão (2026-05-25): bypass com rationale documentado

```yaml
bypass:
  flag: "NOVAIS_FOUNDRY_BYPASS=incident"
  rationale: "founder-solo-pre-team"
  approved_by_implicit: "CEO Novais Digital (single person — Rafael Novaes)"
...
Nenhuma transição executada. `subscriptions/monthly-analysis/promotions.md` será criado pelo primeiro `/novais-digital:promote` que rodar.
...
3. Rodar `/novais-digital:eval --module=monthly-analysis` (ou `--artifact_id=monthly-analysis`)
4. Verificar run report em `evals/monthly-analysis/runs/` com `status: pass`
5. Executar `/novais-digital:promote monthly-analysis start_shadow --approver_po=rafael-novaes --approver_promotion_officer=rafael-novaes-acting-as-promotion-officer --bypass=NOVAIS_FOUNDRY_BYPASS=incident:founder-solo-pre-team`
...
A exceção `founder-solo-pre-team` (§4) expira "na primeira contratação de 2ª pessoa técnica/product". Com **the CEO (CEO)** e **dev frontend (dev)** no projeto, a condição foi atingida. A aprovação cruzada real está disponível e passa a ser **obrigatória** (sem bypass):
...
1. 🔴 **Gate 4**: rodar `/novais-digital:eval` fresco (≤7 dias) com o `prompt_hash` atual (pós-flip) — o de 25/05 não vale mais.
