# Padrões de Orquestração — Novais Digital Foundry

Catálogo de referência de padrões de orquestração endossados pelo Foundry, mais anti-padrões a evitar. Leia isto antes de adicionar um novo `/novais-digital:*` command que coordene múltiplos Guardians, ou antes de criar um Guardian que "envolva" outros.
...
Adaptado de `agent-skills/references/orchestration-patterns.md` por addyosmani, estendido para o vocabulário Foundry.
...
Um `/novais-digital:*` command que envolve um Guardian com o contexto do projeto. Poupa o usuário de re-explicar o workflow toda vez.

```
/novais-digital:spec → @po-guardian (valida C2) → spec.md persistida
```

**Use quando:** a mesma invocação de único Guardian acontece repetidamente com o mesmo setup.

**Exemplos neste repo:** `/novais-digital:diagnose`, `/novais-digital:eval`, `/novais-digital:sla-threshold`.
...
/novais-digital:promote ──────  ┼─→ @unit-economist        (C3 viabilidade)   ├→ merge → go/no-go + signature
...
**Exemplos neste repo:** `/novais-digital:promote` (fan-out de 5 Guardians + merge).
...
```
usuário executa:
/novais-digital:diagnose → /novais-digital:spec → /novais-digital:unit-economics → /novais-digital:sla-threshold
→ /novais-digital:plan → /novais-digital:tasks → /novais-digital:implement → /novais-digital:eval
→ /novais-digital:pre-merge-check → /novais-digital:promote → /novais-digital:audit-monthly
```
...
**Exemplos Foundry:** "Encontre todos os artefatos no manifest sem linked_principles", "Summarize o que as 5 audits dizem sobre drift de custo".
...
Para `/novais-digital:pre-merge-check`, múltiplas verificações de princípio rodam em paralelo sobre o diff, cada uma focada em um eixo diferente.
...
```
/novais-digital:work → guardian-router → "isto precisa de review" → @po-guardian → router (parafraseia) → usuário
```
...
- Replica o trabalho que os slash commands e `@foundry-router` já fazem

**O que fazer em vez disso:** adicionar ou refinar um `/novais-digital:*` command. Documentar mapeamento de intenção → command em `CLAUDE.md` e no `@foundry-router`.
...
Um agente que chama `/novais-digital:diagnose`, depois `/novais-digital:spec`, depois `/novais-digital:plan` em nome do usuário, automaticamente.
...
**O que fazer em vez disso:** manter o usuário como orquestrador. Documentar a sequência recomendada em `HELLO.md` e deixar o usuário invocá-la. `@foundry-router` classifica a intenção e sugere o próximo command — não executa a sequência toda.

---

### D. Árvores Profundas de Guardian

`/novais-digital:promote` chama um `pre-promote-coordinator` que chama um `quality-coordinator` que chama `@artifact-architect`.
...
Guardians do Foundry ficam em `.claude/agents/`. São auto-descobertos pelo Claude Code quando a sessão inicia. Sem configuração de path necessária.
...
1. Você usou o padrão pelo menos duas vezes em trabalho real no Foundry
