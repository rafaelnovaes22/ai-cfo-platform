...
Coerente com ADR-002 §2.2.3 (eixos do benchmark). Promoção via `/acme:promote` exige `agreement_rate ≥0.95` em pelo menos 30 análises observadas em 14 dias consecutivos.
...
Execução: `/acme:eval --module classification --model gemini-2.0-flash`.
...
Re-cálculo formal vinculado a `docs/onda-0/unit_economics.md`. Disparo automático de recalc sempre que `src/llm/router.ts` mudar (Gate 2 de `/acme:promote`).
...
| **Cliente em ASSISTED corrige em massa de forma errada** (poluindo flywheel) | Baixa | Alto — degrada dataset de fine-tune | Audit mensal (`/acme:audit-monthly`) sample 5–10% das correções; outliers (>3 correções/dia/cliente) marcados para revisão |
