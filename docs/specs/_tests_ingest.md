...
6. **Eval suite ≥10 casos por outcome (C2/foundry convention)** — não é teste unitário, é eval. Ficará em `evals/ingest/cases/` separado.
...
10. **Cláusula de outcome contratual (C2)** — a spec está em status stub e não declara a "cláusula de outcome" formal cobrável. Quando `/novais-digital:spec --module ingest` rodar, novas regras virão e este test plan deve ser revisitado.
