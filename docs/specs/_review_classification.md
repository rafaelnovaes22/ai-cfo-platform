...
> Revisão consolidada do módulo `classification` (Onda 1, Tier B) contra a spec aprovada e os 8 princípios da Constitution Foundry v0.2.0.
...
- [SUGGESTION] `buildSystemPrompt` poderia incluir hash de versão (`PROMPT_VERSION = "1.0.0"`) emitido como metadata em `callLlm` — facilita auditoria mensal de drift via `/novais-digital:audit-monthly`.
