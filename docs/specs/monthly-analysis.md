...
spec_version: "0.2.0"
foundry_command_version: "spec@0.2.0"
...
canonical_migration:
  ...
  reason: "Foundry canonical path required for /novais-digital:promote, /novais-digital:plan, /novais-digital:eval — original kept as runtime reference"
---
...
>
> **Nota de migração** (2026-05-25): este arquivo é a versão canônica Foundry da spec. O original em [`src/skus/monthly-analysis/spec.md`](../../src/skus/monthly-analysis/spec.md) permanece como referência inline ao código. Mudanças contratuais (cláusula de outcome, categorias, exemplos) **devem ser feitas aqui** e propagadas via PR (o hash é checado por `/novais-digital:promote`).
...
`outcome_clause_hash: 3b9278b7825aaa9e` (sha256:16 da cláusula acima — checked por `/novais-digital:promote` gate 1 contra `prompt.outcome_clause_hash`)
...
**Trace coverage gate**: 100% das chamadas LLM em produção devem ter trace_id. Sem trace → outcome não conta (`/novais-digital:audit-monthly` flag).
...
- Suite E2E do SKU **ainda não criada** — bloqueia gate 4 de `/novais-digital:promote`
...
| 0.2.0 | 2026-05-25 | Migração para Foundry canonical path (`docs/specs/`); frontmatter Foundry-compatible (c2_validation, outcome_clause_hash, outcome_categories, linked_*); pipeline 4.1 atualizado para Gemini Flash (LGPD) + financial-qa-review node | Rafael Novaes + Claude Sonnet 4.6 |
