...
- **Eval suite mínima (≥10 casos por outcome)** — exigência genérica da spec gerada via `/novais-digital:spec`. Esta suíte são testes unit/int, não eval suite. **Pertence a** `evals/classification/cases/*.yaml` (a ser criada via `/novais-digital:eval`).
...
- **Promoção SHADOW → ASSISTED → AUTONOMOUS** — comando `/novais-digital:promote` valida modo; fora do escopo do classificador.
- **Cláusula formal de outcome (C2)** — a spec atual é stub e não declarou cláusula contratual. Quando preenchida via `/novais-digital:spec`, casos de aceitação literal devem virar testes adicionais.
