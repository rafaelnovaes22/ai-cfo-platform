...
> Estes números são contrato — `/acme:promote` lê deste frontmatter. Mudança exige nova ADR + reaprovação.
...
Runner: `/acme:eval --module ingest` compara saída do parser com gabarito via `exact_match` em `outcome` + `range_match` em counts.
