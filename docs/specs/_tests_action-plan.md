...
3. **Eval suite mínima (≥10 casos por outcome)**: declarada na spec stub como TBD. Não testada aqui — pertence a `evals/action-plan/cases/` e roda via `/acme:eval`.
...
8. **Promoção entre modos (SHADOW → ASSISTED → AUTONOMOUS)**: gerenciado por `/acme:promote`, fora do escopo do módulo `action-plan`.
