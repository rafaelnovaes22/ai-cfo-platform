...
1. **Eval suite ≥10 casos por outcome (C3 + C4)** — a spec stub não declara cases; quando spec evoluir de `stub` para detalhada via `/acme:spec --module export`, criar `evals/export/cases/*` e suite `npm run eval -- export`. Não cabe em Vitest.
