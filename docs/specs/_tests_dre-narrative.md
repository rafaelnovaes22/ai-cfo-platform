...
2. **Threshold X% explícito da anomalia** — a spec stub fala em "variação >X% vs. mês anterior" mas não declara X. Sem número canônico não há como escrever assert determinístico. Ação: `/novais-digital:spec --module dre-narrative` precisa fixar X (sugestão: 30% MoM).
...
7. **Unit economics (C3) — custo ≤25% do preço** — não testável em unit test; pertence a `docs/onda-0/unit_economics.md` e `/novais-digital:unit-economics`. Apenas validamos que `costCents` é acumulado corretamente.
