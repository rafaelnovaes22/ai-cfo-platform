---
# Agent Soul — Aicfo (consumer Foundry)
# Carregado no slot #1 do system prompt por hooks/session-start/foundry-context.sh
...
foundry_version_required: 0.21.0
---

# Agent Soul — Aicfo

> CFO-IA / plataforma de gestão financeira inteligente para PMEs.
> Produto self-serve do guarda-chuva **Novais Digital SaaS²**.
...
- **Frontend**: repositório backend-only — contratos OpenAPI 3.1 + Zod expostos para qualquer cliente consumir
- **LLM**: provider agnóstico via `src/llm/router.ts`; adapters isolados em `src/llm/adapters/` (C7)
- **Tracing**: LangSmith (canônico para tracing LLM — C6; integração nativa com LangGraph)
...
- **TDD-first em módulos novos**: `test_agent` (AIOS) gera testes RED antes do backend (Foundry-10)
