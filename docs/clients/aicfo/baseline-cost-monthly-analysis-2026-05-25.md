...
foundry_command_version: "unit-economics@0.1.0"
created_at: "2026-05-25"
recalc_at: "2026-05-25"
last_recalculated_reason: "consolidação de premissas pós-migração LGPD (ADR-010) + criação do arquivo canônico Foundry"
...
> Arquivo canônico Foundry derivado de `docs/onda-0/unit_economics.md` (D5 — múltiplos recálculos). Função: alimentar `/novais-digital:promote` Gate 2 (C3) com `c3_check.status` e `cost_per_outcome`. **Fonte de verdade do cálculo continua em [`docs/onda-0/unit_economics.md`](../../onda-0/unit_economics.md)**; este arquivo consolida o estado vigente em formato Foundry-compatible.
...
A IA custa **3.408× menos** que o substituto humano comparável. Isso satisfaz com folga gigante o requisito `c4_thresholds.cost_per_outcome_max <= human_cost_per_unit` do gate 3 de `/novais-digital:promote`.
...
| Customização de prompt por tenant (proibido por C8, mas se quebrasse) | ~3-5% (escala linear no nº de variantes) | ⚠️ ainda viável mas anti-Foundry |
...
## 5. Triggers de recálculo (per ADR-002 §2.2.1 + Foundry `unit-economics-recalc` hook)
...
- [ ] `prompt_hash` mudar em qualquer um dos módulos do pipeline (`/novais-digital:eval` deve ter `prompt_hash_at_recalc` matching ou disparar gate failure)
...
- [ ] Validação empírica via SHADOW de 14 dias com 5-10 PMEs (pendente — bloqueio do gate 4 do /novais-digital:promote, **não** do gate 2)

**Aprovado para Gate 2 do `/novais-digital:promote start_shadow`**: ✅ `c3_check.status: viable`.
