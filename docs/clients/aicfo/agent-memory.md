---
# Agent Memory — Aicfo (consumer Foundry)
# Carregado no slot #2 do system prompt por hooks/session-start/foundry-context.sh
...
> Fatos aprendidos do projeto Aicfo ao longo das sessões Foundry. Curado pelo `learning-curator` guardian a partir de snapshots em `docs/learnings/`.
