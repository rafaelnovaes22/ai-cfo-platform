...
> **Bloqueia**: promoção da Onda 1 para ASSISTED (Gate 2 de `/novais-digital:promote` exige `recalc_unit_economics_required` resolvido)
...
|---|---|---|
| **Nova wave habilitada** | Antes de qualquer módulo de Onda 2+ entrar em SHADOW | Tech Lead via `/novais-digital:plan` |
...
A eval suite vive em `evals/{module}/cases/` e é executada via `/novais-digital:eval --module {module} --model {candidate}`.
...
|---|---|---|---|
| **Qualidade** | pass_rate da eval suite | execução automática do `/novais-digital:eval` | ≥ baseline atual − 1pp |
...
3. Eval suite executada via /novais-digital:eval para baseline (modelo atual) e candidato
...
7. Toda mudança em src/llm/router.ts dispara recalc_unit_economics_required
   (Gate 2 do /novais-digital:promote) → docs/onda-N/unit_economics.md re-validada
```
...
- ❌ Definir o conteúdo de cada eval suite — escopo do `/novais-digital:eval` por módulo (Onda C dos fixes)
