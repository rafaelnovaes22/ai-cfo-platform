...
> **Template**: `templates/adr-reviewer-runtime.template.md` (Forge v0.15.0)
> **Substitui**: nenhum
...
O Acme Forge define o **reviewer DeepAgent** como camada de auditoria mensal externa (decisões F17/F18).
O Forge decide **o quê** auditar (10 SKILL.md em `reviewer/deepagents/skills/`).
...
O reviewer roda como job `audit-monthly` em workflow dedicado `.github/workflows/forge-audit.yml`,
agendado para o primeiro dia útil de cada mês (`cron: '0 9 1 * *'`), com `workflow_dispatch` para auditorias sob demanda.

**Justificativa**: Sem infra extra. Log centralizado. Reproducível. O projeto já tem `forge-validate.yml` rodando — extensão natural. Para um projeto em SHADOW ainda sem clientes reais, infra extra (cron em servidor dedicado) não se justifica.
...
| Destino do relatório | `docs/forge/audits/YYYY-MM.md` |
...
- Workflow `forge-audit.yml` será criado em `.github/workflows/`
...
- Relatórios ficam em `docs/forge/audits/` (criar diretório antes da primeira auditoria)
