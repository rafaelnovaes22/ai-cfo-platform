...
3. Validação manual em `app-stg.example.com` com tenants sintéticos
...
| Dia | Atividade |
|---|---|
| **D1** | Criar environment `staging` no Railway, espelhar serviços de produção, configurar domínios `api-stg.example.com` e `app-stg.example.com` |
...
- [ ] Push para branch `staging` dispara deploy em `api-stg.example.com`
...
- Validação manual de mudanças de UI antes de prod (dev frontend pode testar a tela de "O que o Aicfo aprendeu" em staging antes do cliente ver)
